...
async function handleFavIcon(server, options) {
  ...
  // look in CWD/static
  const icon = await xaa.try(() => readFile(Path.resolve(options.favicon || "static/favicon.ico")));
...
async function setupSubAppHapiRoutes(server, pluginOpts) {
  ...
  return await setupRoutesFromFile(srcDir, server, pluginOpts);
}
