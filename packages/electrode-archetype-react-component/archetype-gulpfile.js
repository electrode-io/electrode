...
function checkFrontendCov(minimum) {
  ...
  return exec(`istanbul check-coverage 'coverage/client/*/coverage.json' --config=${archetype.devPath}/config/istanbul/.istanbul.${minimum}yml`);
}
...
const tasks = {
  ...
  "build-dist-min": {
    ...
    task: () => exec(`webpack --config ${archetype.devPath}/config/webpack/webpack.config.js --colors`)
  },
  "build-lib": {
    dep: ["~production-env"],
  ...
  "build-lib:copy-flow": () => exec(`node ${archetype.devPath}/scripts/copy-as-flow-declaration.js`),
  "build-lib:flatten-l10n": () => exec(`node ${archetype.devPath}/scripts/l10n/flatten-messages.js`),
  ...
  "check-dep": () => exec(`ecd -f package.json --cf ${archetype.devPath}/config/dependencies/check.json -w`),
  ...
  "lint-react-src": () => exec(`eslint --ext .js,.jsx -c ${archetype.devPath}/config/eslint/.eslintrc-react src --color`),
  "lint-react-test": () => exec(`eslint --ext .js,.jsx -c ${archetype.devPath}/config/eslint/.eslintrc-react-test test/client --color`),
  "lint-scripts": () => exec(`eslint --ext .js -c ${archetype.devPath}/config/eslint/.eslintrc-base scripts --color`),

  "npm:prepublish": ["build-lib", "build-dist-min"],

  "server-test": () => exec(`webpack-dev-server --port 3001 --config ${archetype.devPath}/config/webpack/webpack.config.test.js --colors`),
  ...
  "test-frontend": () => exec(`karma start ${archetype.devPath}/config/karma/karma.conf.js --colors`),
  "test-frontend-ci": () => exec(`karma start --browsers PhantomJS,Firefox ${archetype.devPath}/config/karma/karma.conf.coverage.js --colors`),
  "test-frontend-cov": () => exec(`karma start ${archetype.devPath}/config/karma/karma.conf.coverage.js --colors`),
  "test-frontend-dev": () => exec(`karma start ${archetype.devPath}/config/karma/karma.conf.dev.js --colors`),
  "test-frontend-dev-watch": () => exec(`karma start ${archetype.devPath}/config/karma/karma.conf.watch.js --colors --browsers Chrome --no-single-run --auto-watch`)
};
