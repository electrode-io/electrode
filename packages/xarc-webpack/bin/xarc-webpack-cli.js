...
(function() {
  ...
  // webpack-cli is a dependency, so we know it's local to us

  const webpackCliPkg = require("webpack-cli/package.json");

  if (!webpackCliPkg.version.startsWith("3.")) {
    throw new Error("This xarc-webpack-cli is only compatible with webpack-cli version 3");
  }

  const requireAt = require("require-at")(require.resolve("webpack-cli"));

  const { NON_COMPILATION_ARGS } = requireAt("./utils/constants");

  requireAt("v8-compile-cache");

  const ErrorHelpers = requireAt("./utils/errorHelpers");
  ...
  if (NON_COMPILATION_CMD) {
    return requireAt("./utils/prompt-command")(NON_COMPILATION_CMD, ...process.argv);
  }

  const yargs = requireAt("yargs").usage(`webpack-cli ${webpackCliPkg.version}
...
For more information, see https://webpack.js.org/api/cli/.`);
  ...
  requireAt("./config/config-yargs")(yargs);
  ...
  yargs.parse(process.argv.slice(2), (err, argv, output) => {
    ...
    let options;
    try {
      options = requireAt("./utils/convert-argv")(argv);
    } catch (err) {
    ...
    function processOptions(options) {
      ...
      if (typeof outputOptions.colors === "undefined")
        outputOptions.colors = requireAt("supports-color").stdout;
