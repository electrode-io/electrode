...
const Footer = (props: { title: any; }) => {
  return (
    <footer className={classNames(custom["container-fluid"], custom["text-center"])} style={{ backgroundColor: '#f2f2f2' }}>
      <p>{props.title}</p>
      <form className={custom["form-inline"]}>
        Get deals:
        <input type="email" className={custom["form-control"]} size="50" placeholder="Email Address" />
...
const Component = connect((state: any) => state, (dispatch: any) => ({ dispatch }))(Footer);
